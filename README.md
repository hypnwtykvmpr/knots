...
removes the temp directories on exit. To re-record (Unix-only: requires
`make`, a POSIX shell, and asciinema — not supported on native Windows):
...
```bash
curl -fsSL https://raw.githubusercontent.com/hypnwtykvmpr/knots/main/install.sh |
  KNOTS_GITHUB_REPO=hypnwtykvmpr/knots sh
```

```powershell
$env:KNOTS_GITHUB_REPO = 'hypnwtykvmpr/knots'
iwr https://raw.githubusercontent.com/hypnwtykvmpr/knots/main/install.ps1 -UseBasicParsing | iex
```
...
6. Create a hierarchy of knots via `kno new "<title>"` for parent knots, `kno q "<title>"` for child knots and `kno edge add <parent-id> parent_of <child-id>` for edges
...
while ($true) {
  kno poll --claim
  if ($LASTEXITCODE -ne 0) { Start-Sleep -Seconds 30; continue }
  # ... do the work described in the prompt ...
  # ... run the completion command from the output ...
}
```
...
Or in PowerShell:
```powershell
$lease = kno lease create --nickname "my-session" --agent-name "claude-code" `
  --model "opus" --model-version "4.6" --json | ConvertFrom-Json
...
[ci-badge]: https://github.com/hypnwtykvmpr/knots/actions/workflows/ci.yml/badge.svg
[ci-url]: https://github.com/hypnwtykvmpr/knots/actions/workflows/ci.yml
[coverage-badge]: https://codecov.io/gh/hypnwtykvmpr/knots/graph/badge.svg?branch=main
[coverage-url]: https://codecov.io/gh/hypnwtykvmpr/knots
