...
pub(super) fn parse_index_scope_data(
    ...
    let raw = object
        .get("scope")
        .ok_or_else(|| invalid_event(path, "missing 'scope' object field"))?;
    serde_json::from_value(raw.clone())
        .map_err(|err| invalid_event(path, &format!("invalid scope payload: {}", err)))
}
...
pub(super) fn build_index_upsert(
    ...
    let lease_data = existing
        ...
        .unwrap_or_default();
    let scope_data = if params.data.contains_key("scope") {
        parse_index_scope_data(params.data, params.absolute_path)?
    } else {
        existing
            .as_ref()
